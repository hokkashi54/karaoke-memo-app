...
const SongCard = ({ song, onEdit, onDelete, compact = false, isSelectable = false, isSelected = false, onToggleSelect = null }) => {
  return (
    <div className={`bg-slate-800 rounded-lg p-3.5 border shadow-sm transition-colors ${isSelectable && isSelected ? 'border-blue-500 bg-blue-900/20' : 'border-slate-700 hover:border-slate-600'}`}>
      <div className="flex justify-between items-start">
        {isSelectable && (
          ...
          >
            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-blue-500 border-blue-500' : 'border-slate-500 bg-slate-800'}`}>
...
// --- Bulk Tag Modal ---
const BulkTagModal = ({ isOpen, onClose, selectedCount, onAddTags }) => {
  const [tags, setTags] = useState('');
  const [mode, setMode] = useState('append'); // 'append' or 'overwrite'
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="タグを一括追加">
      <div className="space-y-4">
        <div className="bg-slate-900/50 p-3 rounded-lg border border-blue-500/30 flex items-start gap-3">
          <AlertCircle size={18} className="text-blue-400 mt-0.5 flex-shrink-0" />
          <div className="text-sm text-slate-300">
            選択中の <strong className="text-white">{selectedCount}曲</strong> にタグを追加します。
          </div>
        </div>
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-2">追加方法</label>
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => setMode('append')}
              className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-1 transition-colors ${mode === 'append' ? 'bg-blue-600/20 border-blue-500 text-blue-300' : 'bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-700'}`}
            >
              <ArrowDownToLine size={20} />
              追加する (追記)
              <span className="text-[10px] font-normal opacity-70">既存のタグを残す</span>
            </button>
            <button
              onClick={() => setMode('overwrite')}
              className={`p-3 rounded-lg border text-sm font-bold flex flex-col items-center gap-1 transition-colors ${mode === 'overwrite' ? 'bg-red-600/20 border-red-500 text-red-300' : 'bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-700'}`}
            >
              <ArrowUpToLine size={20} />
              上書きする
              <span className="text-[10px] font-normal opacity-70">既存のタグを消去</span>
            </button>
          </div>
        </div>
        ...
        <div className="pt-2">
          <button
            onClick={handleSave}
            disabled={!tags.trim()}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold py-3.5 rounded-lg flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-lg shadow-blue-900/20"
          >
            <Save size={18} />
            適用する
          </button>
...
export default function App() {
  ...
  const [isCompactMode, setIsCompactMode] = useState(() => {
    try {
      const saved = localStorage.getItem('karaoke-compact-mode');
      return saved ? JSON.parse(saved) : true; // Default to true on small screens typically, but let's user decide (true = compact)
    } catch (e) {
      return false;
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('karaoke-compact-mode', JSON.stringify(isCompactMode));
  }, [isCompactMode]);
  ...
  const getSortedSongs = (sourceSongs) => {
    ...
    if (searchQuery) {
      ...
      if (searchQuery === 'タグ無し' || searchQuery === 'タグなし') {
        result = result.filter(s => !s.tags || s.tags.trim() === '');
      } else {
  ...
  const toggleSongSelection = (id) => {
    const newMap = new Set(selectedSongIds);
    if (newMap.has(id)) {
      newMap.delete(id);
    } else {
      newMap.add(id);
    }
    setSelectedSongIds(newMap);
  };

  const handleBulkAddTags = (tagsString, mode) => {
    if (!tagsString.trim() || selectedSongIds.size === 0) return;

    const newTags = tagsString.split(',').map(t => t.trim()).filter(t => t);
    if (newTags.length === 0) return;

    setSongs(prev => prev.map(song => {
      if (!selectedSongIds.has(song.id)) return song;

      if (mode === 'overwrite') {
        return { ...song, tags: newTags.join(', ') };
      } else {
        // Append mode
        const existingTags = song.tags ? song.tags.split(',').map(t => t.trim()).filter(t => t) : [];
        const combinedSet = new Set([...existingTags, ...newTags]);
        return { ...song, tags: Array.from(combinedSet).join(', ') };
      }
    }));
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans pb-24 flex flex-col overscroll-none" style={{ overflowAnchor: 'none' }}>
      ...
      {
        isBulkSelectMode ? (
          <div className="fixed bottom-[64px] left-0 right-0 bg-slate-800/95 backdrop-blur-sm border-t border-slate-700 p-4 z-20 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-2 duration-300">
            <div className="max-w-3xl mx-auto flex items-center justify-between">
              <div className="text-sm font-bold text-white">
                {selectedSongIds.size} 曲を選択中
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    if (selectedSongIds.size === sortedSongs.length) {
                      setSelectedSongIds(new Set());
                    } else {
                      setSelectedSongIds(new Set(sortedSongs.map(s => s.id)));
                    }
                  }}
                  className="bg-slate-700 hover:bg-slate-600 text-slate-300 px-4 py-2 rounded-lg text-sm font-bold transition"
                >
                  {selectedSongIds.size === sortedSongs.length ? '全選択解除' : 'すべて選択'}
                </button>
                <button
                  onClick={() => setIsBulkTagModalOpen(true)}
                  disabled={selectedSongIds.size === 0}
                  className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-lg shadow-blue-900/20 transition flex items-center gap-2"
                >
                  <Plus size={16} />
                  タグを一括追加
                </button>
